...
To Install:
Copy this file into the Google Spreadsheet Script Editor.
Set up two triggers for "cryptoUpdateSheet". 1) Time driven every 5+ minutes, 2) On opening the document.
...
/** 
 * Return the Coin Market Cap price.
 */
function priceURL(coin,fiat)
{
  return "https://api.coinmarketcap.com/v1/ticker/" + coin + "/?convert="+fiat;
}

/** 
 * Update Spreadsheet.
 */
function cryptoUpdateSheet()
{
  ...
  for (i=1;i<=rws;i++) {
    for (j=1;j<=cls;j++) {
      ...
      try
      {
        cellFormla = r.getCell(i,j).getFormula();
        if (cellFormla.indexOf("=cryptoPrice")!=-1)
         {
            r.getCell(i, j).setValue(0);           
            SpreadsheetApp.flush();
            r.getCell(i, j).setValue(cellFormla);
         }
      }
      catch (err) {continue;}
    }
 ...
 * coin = ethereum,factom,bitcoin....
 * fiat = USD,GBP,EUR..... (defaults to USD)
 * dateLoc = left,right,top,botom (optional)
 */

function cryptoPriceS(coin,fiat,dateLoc)
{
    ...
    else if (dateLoc=="bottom")
    {
      return [price,date];
    }
    else if (dateLoc=="left")
    {
      return [[date,price]];   
    }
    else if (dateLoc=="right")
    {
      return [[price,date]];
    }
    else  
...
/** 
 * Executes cryptoPriceS twice, if first fails
 */

function cryptoPrice(coin,fiat,dateLoc)
{
   try
   {
     return cryptoPriceS(coin,fiat,dateLoc);
   }
   catch(err)
   {
     return cryptoPriceS(coin,fiat,dateLoc);
